...
class CustomAuthManager {
  ...
  Future signOut() async {
    ...
    simplexAuthUserSubject.add(
      SimplexAuthUser(loggedIn: false),
    );
    persistAuthData();
  }

  Future<SimplexAuthUser?> signIn({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
  ...
  SimplexAuthUser? _updateCurrentUser({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
    ...
    final updatedUser = SimplexAuthUser(
      loggedIn: true,
      uid: authUid,
      userData: userData,
    );
    simplexAuthUserSubject.add(updatedUser);
    persistAuthData();
    return updatedUser;
  }
  ...
  Future initialize() async {
    ...
    try {
      ...
      userData = _prefs.getString(_kUserDataKey) != null
          ? DtUserStruct.fromSerializableMap(
